...
public class A_and_B {
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            ...
            long costA = minCostToGroup(s, 'a');
            long costB = minCostToGroup(s, 'b');
            out.append(Math.min(costA, costB)).append('\n');
        }
    ...
    private static long minCostToGroup(String s, char ch) {
        ArrayList<Integer> pos = new ArrayList<>();
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == ch) pos.add(i);
        }
        int k = pos.size();
        if (k <= 1) return 0L;
        ...
        long[] arr = new long[k];
        for (int i = 0; i < k; i++) arr[i] = (long)pos.get(i) - i;

     
        long median = arr[k / 2];

        long cost = 0L;
        for (int i = 0; i < k; i++) cost += Math.abs(arr[i] - median);
        return cost;
    }
}
/*
